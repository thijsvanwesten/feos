...
impl<D: DualNum<f64> + Copy> HelmholtzEnergyDual<D> for ChainMie {
    ...
    fn helmholtz_energy(&self, state: &StateHD<D>) -> D {
        ...
        for i in 0..n {
            ...
            let y_wca_sigma = y_wca_aroundcontact_mix(D::one(), &p, eta, &state.partial_density, state.temperature, &d, i, i);
            ...
            + t_st.recip()*(par[9] + par[10]*nu_inv + par[11]*nu_inv*nu_inv);
            let aa = (par[14] + par[15]*nu_inv).abs();
            let bb = par[16]*nu_inv + par[17]*nu_inv*nu_inv + par[18]*nu_inv.powi(3);
            ...
            if p.m[i] > 2.0 && l_tpt1y {
                ...
                let bfac = ( (bondlength_sq*4.0 + 1.0) / (bondlength_sq*2.0) ).sqrt() * p.sigma[i];
                let bfac_st = ( (bondlength_sq.powi(2)*4.0 + bondlength_sq) / (bondlength_sq*5.0) ).sqrt();
...
fn packing_fraction_c_ij<D: DualNum<f64> + Copy>(
    ...
    -2.70748992e-01, -7.57290826e+01,  2.96315756e+00, -2.80211182e+01,
...
fn y_hf<D: DualNum<f64> + Copy>(
    ...
    xx: D) -> D {
        ...
        //-------------------
        let mut dlog_y0 = Array1::<D>::zeros(n);
        for i in 0..n {
            for k in 0..n{
                let dik = (d[i]+d[k])*0.5;
        ...
        let p = UVParameters::new_binary(
            vec![
                PureRecord::new(
                    ...
                    1.0,
                    UVRecord::new(
                        3.0, 12.0, 6.0, 1.0, 1.0, None, None, None, None, None, None, None,
                    ),
                ),
                PureRecord::new(
                    Identifier::default(),
                    1.0,
                    UVRecord::new(
                        2.0, 12.0, 6.0, 1.5, 1.25, None, None, None, None, None, None, None,
                    ),
                ),
            ],
            None,
