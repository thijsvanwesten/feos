...
use crate::uvtheory::ufwca_tpt::hard_sphere_wca::{diameter_wca, dimensionless_diameter_q_wca, packing_fraction};
use crate::uvtheory::ufwca_tpt::hard_sphere_wca::{packing_fraction_a_ij, packing_fraction_b_ij};
...
#[derive(Clone)]
pub struct ReferencePerturbationWCAuf;

impl fmt::Display for ReferencePerturbationWCAuf {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "Reference Perturbation")
    }
}

impl ReferencePerturbationWCAuf {
    pub fn helmholtz_energy_density<D: DualNum<f64> + Copy>(
        ...
        for i in 0..n {
            for j in 0..n {
                ...
                let i0_noldl = ((-eta_a * 0.5 + 1.0) / (-eta_a + 1.0).powi(3) - 1.0)
                    * (-q_st_3 + rs_st_3)
...
mod test {
    ...
    fn test_delta_a0_wcauf_pure() {
        ...
        let a = ReferencePerturbationWCAuf.helmholtz_energy_density(&p, &state) / reduced_density;

        assert_relative_eq!(a, 0.35951228642704941, epsilon = 1e-9);
    }
    #[test]
    fn test_delta_a0_wcauf_mixture() {
    ...
    #[test]
    fn test_delta_a0_wcauf_mixture_different_sigma_chainlength() {
        let molefracs = dvector![0.4, 0.6];
        let reduced_temperature = 1.0;
