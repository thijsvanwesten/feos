use crate::uvtheory::ufwca_tpt::hard_sphere_wca::{
    diameter_wca, dimensionless_diameter_q_wca, packing_fraction, 
    packing_fraction_a_ij,packing_fraction_b_ij, zeta,
};
use crate::uvtheory::eos::ChainContribution;
use crate::uvtheory::parameters::*;
// use crate::uvtheory::ufwca_tpt::attractive_perturbation_wca::one_fluid_properties;
...
#[derive(Clone)]
pub struct ChainMieuf {
    pub chain_contribution: ChainContribution,
}

impl fmt::Display for ChainMieuf {
    ...
    pub fn helmholtz_energy_density<D: DualNum<f64> + Copy>(
        ...
        for i in 0..n {
            ...
            if p.m[i] > 2.0 && l_tpt1y {
                ...
                // old parameterization, uv-theory for monomer, old parmeterization ywca and psi (with artefacts)
                // let par3 = [
                //     8.187740E-004,
                //     1.247571,
                //     8.507247,
                ...
                // 2026 parameterization, uf-theory for monomer, new parameterization for ywca and psi (without artefacts)
                let par3 = [
                    5.1405713374909746E-004, 1.2436996857248634, 7.6831111497125866, 
                    0.19781669589217515, 0.90249497230960340, -2.0727890440519694, 
                    -8.1796094792671258E-005, 2.8919464274353469E-002, 1.2904600276194838, 
                    1.0329245105525902E-003
                ];
...
pub(crate) fn gmie_aroundcontact_mix<D: DualNum<f64> + Copy>(
    r_st: f64,
    p: &UVTheoryPars,
    eta: D,
    ...
    // new 2026 parameterization without artefacts at low temperature.
    //-------
    let alpha = mean_field_constant(rep, att, 1.0);
    let tc = 1.173*alpha + 0.254;
    let tr = t_st/tc;
        
    let c_phi = [  
        -0.3684348,    2.23828659,  -1.56493915,   0.53679344,  -3.76923298, 
        7.04316141,   2.65406345,  -3.13129651,  -7.15211417,  -0.2735659, 
        8.04381986,  -7.18075329,  -5.21169156,  -3.56078744,   9.82820739, 
        3.95850073,   2.90764418,   1.12306243,   0.30297612,   5.27958159, 
        -2.20912905,  -0.32175551,   5.81258979,  10.85537832, -14.40267201, 
        0.22150361,   8.57978098 
    ];
    let mut c1=D::zero();
    let mut c2=D::zero();
    let mut k=0;
    for i in 0..3 {
        for j in 0..3 {
            c1 += D::one()*c_phi[k] * alpha.powi(j) / (D::one()+tr).powi(i);
            k += 1;
        }
    }
    for i in 0..3 {
        for j in 0..3 {
            c2 += D::one()*c_phi[k] * alpha.powi(j) / (D::one()+tr).powi(i);
            k += 1; 
        }
    }
    let c3 = (tr*c_phi[25] + 1.0).ln() * (c_phi[23]*alpha + c_phi[24]*alpha.powi(2)) + c_phi[21] + c_phi[22]*alpha;
    let ft = (-(t_st.recip()).tanh().powf(c_phi[26])*1.05 + 1.0) * (c_phi[19] + c_phi[20]*alpha );
    let prefactor = ft * ( -c3*rho_st ).exp() + 1.0;
    
    let phiu = prefactor * (c1*rho_st + c2*rho_st*rho_st ).tanh();
    
    //--------------------
    // CCF Mie fluid at contact or other distance
    //--------------------
...
pub(crate) fn y_hf_pure<D: DualNum<f64> + Copy>(
    x: D,
    eta: D
) -> D {
...
pub(crate) fn y_wca_aroundcontact_mix<D: DualNum<f64> + Copy>(
    r_st: f64,
    p: &UVTheoryPars,
    eta: D,
    partial_density: &DVector<D>,
...
// pub(super)
pub(crate) fn packing_fraction_c_ij<D: DualNum<f64> + Copy>(dhs_st: D, rmin_st: f64, rep: f64, eta: D) -> D {
    ...
    //--------------------------------------------
    // improved 2026 parameterization that does not lead to eta_eff>1
    // AAD / % : 0.0568282750928396
    // MAD / % : 0.834449036708692
    //--------------------------------------------
    let para_ic2 = [
        5.47746,-8.9675,5.77164,-9.02787,16.8451,-9.96569, 
    ...
    let alpha = mean_field_constant(rep, 6.0, 1.0);
    let alpha2 = alpha * alpha;
    let tau2 = tau * tau;

    let c1 = D::one()*(para_ic2[0] + para_ic2[1] / alpha + para_ic2[2] / alpha2) * tau
        + D::one()*(para_ic2[3] + para_ic2[4] / alpha + para_ic2[5] / alpha2) * tau2;

    let c2 = D::one()*(para_ic2[6] + para_ic2[7] / alpha + para_ic2[8] / alpha2) * tau
        + D::one()*(para_ic2[9] + para_ic2[10] / alpha + para_ic2[11] / alpha2) * tau2;

    let c3 = D::one()*(para_ic2[12] + para_ic2[13] / alpha + para_ic2[14] / alpha2) * tau
        + D::one()*(para_ic2[15] + para_ic2[16] / alpha + para_ic2[17] / alpha2) * tau2;

    let c4 = D::one()*(para_ic2[18] + para_ic2[19] / alpha + para_ic2[20] / alpha2) * tau
        + D::one()*(para_ic2[21] + para_ic2[22] / alpha + para_ic2[23] / alpha2) * tau2;

    let c5 = D::one()*(para_ic2[24] + para_ic2[25] / alpha + para_ic2[26] / alpha2) * tau
        + D::one()*(para_ic2[27] + para_ic2[28] / alpha + para_ic2[29] / alpha2) * tau2;
    
    eta * (c1 + c2*eta + c3*eta*eta + c4*eta*eta*eta).tanh().powd(c5)
...
//     //     assertrelative_eq!(y_wca_00, 1.3165641196539490, epsilon=1e-9);
//     //     assertrelative_eq!(y_wca_01, 1.3050790505787109, epsilon=1e-9);
//     //     assertrelative_eq!(y_wca_11, 1.2938262092789194, epsilon=1e-9);
//     //     // assertrelative_eq!(y_wca_00, 1.1709397329447004, epsilon=1e-9);
//     //     // assertrelative_eq!(y_wca_01, 1.1808360388237535, epsilon=1e-9);
//     //     // assertrelative_eq!(y_wca_11, 1.1881410485453159, epsilon=1e-9);        
//     // }
...
//      */
    ...
    fn test_a_ljchain() {
        ...
        dbg!(&a_chain);
        assert_relative_eq!(a_chain, -1.2780710886582514, epsilon = 1e-9);
    }
    ...
    fn test_a_miechain() {
        ...
        let a_chain = chain.helmholtz_energy_density(&p, &state) / reduced_density;
        dbg!(&a_chain);
        assert_relative_eq!(a_chain, -0.91380505076968521, epsilon = 1e-9);
    }
    ...
    fn test_a_chain_mixture() {
        ...
        dbg!(&a);
        assert_relative_eq!(a, -0.28861297259265412, epsilon = 1e-9);
    }
    //  */
}
